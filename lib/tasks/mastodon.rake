    ...
    begin
      ...
      loop do
        ...
        env['DB_NAME'] = prompt.ask('Name of PostgreSQL database:') do |q|
          q.required true
          q.default using_docker ? 'postgres' : 'fedimint_production'
          q.modify :strip
        end

        env['DB_USER'] = prompt.ask('Name of PostgreSQL user:') do |q|
          q.required true
          q.default using_docker ? 'postgres' : 'fedimint'
          q.modify :strip
        end
      ...
      loop do
        ...
        env['SMTP_FROM_ADDRESS'] = prompt.ask('E-mail address to send e-mails "from":') do |q|
          q.required true
          q.default "Fedimint <notifications@#{env['LOCAL_DOMAIN']}>"
          q.modify :strip
        end
        ...
        begin
          ...
          mail = ActionMailer::Base.new.mail to: send_to, subject: 'Test', body: 'Fedimint SMTP configuration works!'
          mail.deliver
          break
        rescue StandardError => e
      ...
      if prompt.yes?('Save configuration?')
        ...
        prompt.say "\n"
        prompt.ok 'All done! You can now power on the Fedimint server 🐘'
        prompt.say "\n"
